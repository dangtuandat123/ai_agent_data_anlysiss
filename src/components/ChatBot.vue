      ...
      <div class="chat-window">
        ...
        <div style="display: flex; justify-content: center; align-items: center; line-height: 0px;">
          <div class="prompt_input" style="width: 60%;">
            ...
            <textarea id="message" name="message" placeholder="Nhập yêu cầu của bạn..." required v-model="prompt"
              @input="adjustRows" :rows="rows"></textarea>
          </div>
        ...
        <div v-show="selectedFiles.length > 0"
          ...
          <div class="list-file-window" style="width: 60%;">
            <div v-for="(file, index) in selectedFiles" :key="index" class="file-item" style="margin: 5px 0px;">
              <span>{{ file.name }}</span>
              <svg @click="removeFile(index)" class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" style="margin-left: 4px;"
                viewBox="0 0 24 24">
